const TILE_LAYERS = {
  ...
  'ESRI Satellite': {
    ...
    options: { attribution: '© Esri, i-cubed, USDA, USGS, AEX, GeoEye, Getmapping, Aerogrid, IGN, IGP, UPR-EGP, and the GIS User Community' },
  },
};
...
export function createLayerControl(map) {
  ...
  for (const name in TILE_LAYERS) {
    ...
    option.innerHTML = `
      <input type="radio" name="base-layer" id="base-${name.replace(/\s/g, '-')}" value="${name}">
      <label for="base-${name.replace(/\s/g, '-')}">${name}</label>
    `;
    baseLayerContainer.appendChild(option);
  }
  
  baseLayerContainer.querySelector('input[value="OpenStreetMap"]').checked = true;
  ...
  for (const name in OVERLAY_LAYERS) {
    ...
    option.className = 'layer-option';
    option.innerHTML = `
      <input type="checkbox" id="overlay-${name.replace(/\s/g, '-')}" value="${name}">
      <label for="overlay-${name.replace(/\s/g, '-')}">${name}</label>
    `;
    overlayContainer.appendChild(option);
  }

  document.getElementById('layer-dropdown-menu').addEventListener('change', (e) => {
  ...
  document.getElementById('layer-dropdown-btn').addEventListener('click', () => {
