  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname.startsWith("/api/")) {
        if (url.pathname.startsWith("/api/upload-handler/")) {
          return handleDirectUpload(request, env, headers);
        }
...
async function handleUploadUrl(request, env, headers) {
  try {
    ...
    // Return a URL that points back to our worker
    const uploadUrl = `/api/upload-handler/${key}`;

    return new Response(
      JSON.stringify({
        uploadUrl,
        key,
        publicUrl: `https://m3d14.pigmap.org/${key}`,
      }),
      {
        headers: { ...headers, "Content-Type": "application/json" },
      }
...
async function handleDirectUpload(request, env, headers) {
  try {
    ...
    if (!key) {
      return new Response(JSON.stringify({ error: "Missing key for upload" }), {
        status: 400,
        headers: { ...headers, "Content-Type": "application/json" },
      });
    }

    // Stream the request body to R2
